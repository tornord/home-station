...
export class Switch {
	...
	toString() {
        return this.name + ": " + this.wakeUp + "-" + this.goToBed + " (" +
            this.weekendWakeUp + "-" + this.weekendGoToBed + ")";
	}
	...
	static getState(w, m, sun, minutes) {
        var { wakeUp, goToBed } = minutes;
        var midday = 12*60;
        if ((w === 6) || (w === 0))
            wakeUp = minutes.weekendWakeUp;
        if (m < midday) {
            if ((w === 6) || (w === 0))
                goToBed = minutes.weekendGoToBed;
        }
        else if ((w === 5) || (w === 6))
            goToBed = minutes.weekendGoToBed;
